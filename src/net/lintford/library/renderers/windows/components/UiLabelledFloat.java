...
public class UiLabelledFloat extends UIWidget {
	...
	public void draw(LintfordCore pCore, TextureBatchPCT pTextureBatch, Texture pUITexture, FontUnit pTextFont, float pComponentZDepth) {
		...
		final float lValueWidth = pTextFont.bitmap().getStringWidth(Float.toString(mValueFloat));
		pTextFont.draw(Float.toString(mValueFloat), x + w - lValueWidth - lHorizontalPadding, y + h / 2 - lTextHeight / 2, pComponentZDepth, ColorConstants.TextEntryColor, 1f, -1);
