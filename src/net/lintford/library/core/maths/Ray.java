...
public class Ray {
	...
	public static float intersects(Rectangle pRect, Ray pRay) {
		...
		if (Math.abs(pRay.direction().y) < EPSILON) {
			...
			if (lMinValue == NO_INTERSECTION || lMinValueY > lMinValue) lMinValue = lMinValueY;
            if (lMaxValue == NO_INTERSECTION || lMaxValueY < lMaxValue) lMaxValue = lMaxValueY;
        }
		
		// having a positive tMin and a negative tMax means the ray is inside the box
        // we expect the intesection distance to be 0 in that case
        if ((lMinValue != NO_INTERSECTION && lMinValue < 0) && lMaxValue > 0) return NO_INTERSECTION;

        // a negative tMin means that the intersection point is behind the ray's origin
        // we discard these as not hitting the AABB
        if (lMinValue < 0) return NO_INTERSECTION;
