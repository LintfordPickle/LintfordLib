...
import net.lintford.library.core.geometry.spritegraph.definition.GraphNodeDef;
...
public class SpriteGraphNodeInst extends Rectangle implements AnimatedSpriteListener {
	...
	public SpriteGraphNodeInst(SpriteGraphInst pSpriteGraphInst, GraphNodeDef pGraphNodeDef) {
		this(pSpriteGraphInst, pGraphNodeDef, 0);
	}

	public SpriteGraphNodeInst(SpriteGraphInst pSpriteGraphInst, GraphNodeDef pGraphNodeDef, float pNodeDepth) {
		this();
		...
		for (int i = 0; i < COUNT_SIZE; i++) {
			GraphNodeDef lNodeDef = pGraphNodeDef.childParts.get(i);
	...
	public void update(LintfordCore pCore, SpriteGraphInst pParentGraph, SpriteGraphNodeInst pParentGraphNode) {
		...
		if (pParentGraphNode != null) {
			...
			lX += pParentGraphNode.x;
			lY += pParentGraphNode.y;
		...
		SpriteSheetDef lNodeSpriteSheet = pCore.resources().spriteSheetManager().getSpriteSheet(spriteSheetNameRef, entityGroupID);
