...
import net.ld.library.ColorConstants;
import net.ld.library.core.debug.GLDebug;
import net.ld.library.core.input.InputState;
import net.ld.library.core.maths.Vector2i;
...
public class DisplayConfig implements Serializable {
	...
	private static final long serialVersionUID = -9094343051587722690L;
	...
	public long onCreateWindow() {
		...
		if (mFullScreen) {
			...
			long lNewWindowID = glfwCreateWindow(lFullScreenWidth, lFullScreenHeight, "", glfwGetPrimaryMonitor(),
					mWindowID);
			if (mWindowID != NULL) {
		...
		// Create a new windowed window
			...
			long lNewWindowID = glfwCreateWindow(mWindowSize.x, mWindowSize.y, "", NULL, mWindowID);
			if (mWindowID != NULL) {
				glfwDestroyWindow(mWindowID);
			}
			mWindowID = lNewWindowID;
